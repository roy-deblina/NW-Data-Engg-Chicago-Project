...
print("Starting transformation pipeline for COVID-19 Cases...")

# 1. CONFIGURATION 
BRONZE_CONTAINER = "bronze"
SILVER_CONTAINER = "silver"
BRONZE_BLOB_NAME = "covid_cases.json" #  CHANGED
SILVER_BLOB_NAME = "clean_covid_cases.parquet" #  CHANGED
...
try:
    ...
    # We only need the key and the zip code for joining
    geo_df = pd.read_sql("SELECT geography_key, zip_code FROM dim_geography", db_engine)
    # Remove duplicate ZIPs (e.g., one ZIP maps to many neighborhoods)
    geo_df = geo_df.drop_duplicates(subset=['zip_code']).copy()
except Exception as e:
...
df.rename(columns={'zip_code': 'zip_code_short'}, inplace=True) # Rename incoming column
df['zip_code'] = df['zip_code_short'].str.slice(0, 5) # Ensure it's just 5 digits
df['week_start'] = pd.to_datetime(df.get('week_start'), errors='coerce')

# Drop rows with no valid ZIP or date
df.dropna(subset=['zip_code', 'week_start'], inplace=True)

# This is the "join" 
# We merge our data (df) with the geography table (geo_df)
df_final = df.merge(geo_df, on='zip_code', how='left')
...
try:
    ...
    print("COVID-19 transformation pipeline finished successfully.")
