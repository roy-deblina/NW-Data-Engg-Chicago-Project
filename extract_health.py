...
logger = get_logger("extract_health")

SODA_DOMAIN = "data.cityofchicago.org"
SODA_DATASET_ID = "iqnk-2tcu"
BLOB_FILE_NAME = "public_health.json"
...
def main():
    logger.info("Starting Public Health extract")
    validate_config()
    try:
        results = download_data()
        json_data = json.dumps(results, indent=2)
        upload_to_azure(json_data)
        logger.info("Public Health extract finished successfully")
    except Exception as exc:
        logger.exception("Public Health extract failed: %s", exc)
        raise SystemExit(1)
